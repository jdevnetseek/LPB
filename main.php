...
require_once KR_PlUGIN_PATH() . '/model/lpb-sl-check.php';
 ...
 * Key Validity.
 */
add_action( 'plugins_loaded', 'SL_Manager::kr_admin_license_check' );
...
function add_admin_message($message, $error = false){
    if($error) {
            ...
            setcookie('wp-admin-messages-error', '@@' . $message, time()+60);
    } else {
            setcookie('wp-admin-messages-normal', '@@' . $message, time()+60);
    }
}     
