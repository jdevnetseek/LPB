...
function admin_kr_page_list_js() {
    ...
    if(!isset($_GET['lastID'])){
     ...
     if( $plan[ 'limits' ] == 3 ){
     	...
     	$plan_name = __( 'LPB Silver License','netseek' );
     	
     	$landing_pages_created  	=  $plan[ 'storedtags' ];
	    $landing_page_remaining 	= ( $plan[ 'limits' ] - $landing_pages_created );
	    
     	if( $landing_page_remaining <= 0 ){
	    	$landing_page_remaining = 0;
	    }else{
	    	$landing_page_remaining = $landing_page_remaining;
	    }
	    
	    if( $landing_page_remaining <= 0 ){
	    	$limit_reach =	__( '(Page limit reached!)', 'netseek' );
	    }else{
	     	$limit_reach =	'';
	    }
	    
	    $dynamic_kr_pages_created 	= $dynamicpage[ 'storedtags' ];
	    $dynamic_kr_pages_remaining = ( $dynamicpage[ 'limits' ] - $dynamic_kr_pages_created );
	    
	    if( $dynamic_kr_pages_remaining <= 0 ){
	    	$dynamic_kr_pages_remaining = 0;
	    }else{
	    	$dynamic_kr_pages_remaining = $dynamic_kr_pages_remaining;
	    }
	     	 
	    if( $dynamic_kr_pages_remaining <= 0 ){
	     	$dynamic_limit_reach =	__( '(Page limit reached!)', 'netseek' );
	    }else{
	     	$dynamic_limit_reach =	'';
    	}
    	
    	$plan_message =  sprintf("%s | <strong>%s</strong> LPB Landing Pages Used - <strong>%s</strong> LPB Landing Page Remaining <strong>%s</strong>",
				     			$plan_name,
				     			$landing_pages_created,
				     			$landing_page_remaining,
				     			$limit_reach
				     	    );
     }elseif ( $plan[ 'limits' ] == 9 ) {
     	$plan_name = __( 'LPB Gold License','netseek' );
     	$landing_pages_created  	=  $plan[ 'storedtags' ];
	    $landing_page_remaining 	= ( $plan[ 'limits' ] - $landing_pages_created );
     	if( $landing_page_remaining <= 0 ){
	    	$landing_page_remaining = 0;
     ...
     }else 
	 {
     	...
     	$plan_message =    "LPB Platinum License | <strong>Unlimited</strong> LPB Landing Pages Allowed ";
     }
    ?>
	  <style>
	#the-list tr td
    ...
    <div style="margin-right: 20px; margin-left: 20px; margin-top: 20px;">
	   ...
	   {
		   ?>
		    <p style="border:1px;padding:10px;background-color: #ffffff;border-left: #03c300 3px solid;"><strong><?php _e( 'Package Status', 'netseek' ) ?></strong>:  <?php _e( $plan_message ,'netseek'); ?></p>
  ...
  <table class="wp-list-table widefat fixed posts">
        ...
        <tbody id="the-list">
         ...
         if(!empty ($result) && is_array($result))
		 {
           ...
           ?>
          <tr style="background-color: #f9f9f9;" class="no-items"><td class="colspanchange">No pages found.</td><td></td><td></td></tr>
