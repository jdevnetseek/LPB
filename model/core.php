...
class KwReplacerCore
{
    ...
    public function SetKrPageTitle( $title )
    {
		...
		$act_deact_status = LPBLicenseManager::check_activated_deactivated_status();
		
    if(isset($get_license_info["status"]) || isset($act_deact_status)):
      if($get_license_info["status"] == "blocked" || $act_deact_status == "deactivated"){
  			 return $title;
  		}
    endif;
